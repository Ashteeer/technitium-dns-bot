...
ttbot --cleanup-zones    # удалить зоны Technitium, сохранив правила (миграция на App)
...
- **🔍 Проверить домен** → опрашивает **фактические зоны Technitium** (через API) и
  показывает, подменяется ли сам домен (и на какой IP), а также перечисляет
  проксируемые **поддомены** запроса с их охватом. Работает и для зон, созданных
  вручную в веб-панели Technitium. Пример: для `googleapis.com` (когда проксируется
  только `robinfrontend-pa.googleapis.com`):

  ```
  🔍 Проверка: googleapis.com
  ❌ googleapis.com — не проксируется

  Проксируемые поддомены:
  • robinfrontend-pa.googleapis.com — проксируется на 10.0.0.53 (домен и поддомены)
  ```
- **⚙️ Настройки**
...
## Режимы работы и переход на App

`config.yaml → manage_zones`:

- **`true`** (по умолчанию) — бот сам создаёт/ведёт авторитативные зоны
  Technitium; подмена работает без внешних компонентов.
- **`false` (режим App)** — бот **не трогает зоны**, только ведёт `rules.yaml`;
  отвечает на запросы (или форвардит наверх) ваш Technitium App, читая этот файл.
  В этом режиме `check_domain` тоже считается по правилам, а не по зонам.

**Полный переход на App** (когда App готов):

```bash
sudo ttbot --update                 # 1. обновить бота
# 2. в config.yaml выставить:  manage_zones: false
sudo ttbot restart                  # 3. бот переходит в режим App (зоны не трогает)
sudo ttbot --cleanup-zones          # 4. удалить старые зоны бота (правила сохранятся)
ttbot --reload                      # 5. (необязательно) сразу обновить rules.yaml
```

После шага 4 Technitium перестаёт быть авторитативным по этим доменам, и запросы
обрабатывает App. Бот продолжает обновлять `rules.yaml` из списков и Telegram.
Креды `technitium` в режиме App используются только командой `--cleanup-zones`.
